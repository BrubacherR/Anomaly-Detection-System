...
static int insertionSortWithIndex(int distances[], size_t k, int indices[]) {
    for (int i = 1; i < k; i++) {
...
static void findKNeighbours(int data[], size_t size, size_t k, int point, int index, int k_neighbours_index[]) {
...
static int reach_distance(int data[], size_t size, size_t k, int point1, int point2_index) {
    ...
    int max = 0;
    int k_neighbours_index[k];
    findKNeighbours(data, size, k, data[point2_index], point2_index, k_neighbours_index);
...
static float localReachabilityDensity(int data[], size_t size, size_t k, int point, int index) {
    int sum = 0;
    int k_neighbours_index[k];
    findKNeighbours(data, size, k, point, index, k_neighbours_index);
...
float calculate_LOF(int data[], size_t size, size_t k, int point) {
		...
		float sum = 0;
		int k_neighbours_index[k];
        findKNeighbours(data, size, k, point, size + 1, k_neighbours_index);
		...
		return sum / (float)(k * local_density);
